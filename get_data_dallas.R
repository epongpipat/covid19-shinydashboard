...
download.file("https://dshs.texas.gov/coronavirus/TexasCOVID-19CumulativeTestsbyCounty.xlsx", temp_path)
df_temp2 <- readxl::read_excel(temp_path, skip = 1, col_names = T) %>%
  rename(region = `County`) %>%
  filter(region == "Dallas") %>%
  as.matrix() %>%
  as.data.frame() %>%
